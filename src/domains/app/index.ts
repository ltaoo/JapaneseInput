...
export class Application<T extends { storage: StorageCore<any> }> extends BaseDomain<TheTypesOfEvents> {
  ...
  /** 平台相关的全局事件 */
  keydown(values: { code: string; value: string; preventDefault: () => void }) {
    const { code, value, preventDefault } = values;
    this.keyboard[code] = true;
    // console.log("[DOMAIN]app - keydown", code, this.keyboard);
    this.emit(Events.Keydown, { code, key: value, preventDefault });
  }
  keyup(values: { code: string; value: string }) {
    const { code, value } = values;
    delete this.keyboard[code];
    if (code === "Escape") {
      this.escape();
    }
    this.emit(Events.Keyup, { code, key: value });
  }
  escape() {
