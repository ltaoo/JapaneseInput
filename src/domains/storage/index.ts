...
export class StorageCore<T extends Record<string, unknown>> extends BaseDomain<TheTypesOfEvents<T>> {
  ...
  removeFrom<K extends keyof T>(key: K, v: Partial<T[K]>) {
    const prevValues = this.get(key) || {};
    if (Array.isArray(prevValues) && Array.isArray(v)) {
      const nextValues = prevValues.filter((vv) => !v.includes(vv));
      this.set(key, nextValues);
      return nextValues;
    }
    if (typeof prevValues === "object" && typeof v === "string") {
      // @ts-ignore
      const nextValues = delete {
        ...prevValues,
      }[v];
      this.set(key, nextValues);
      return nextValues;
    }
    console.warn("the params of removeFrom must be object");
    return prevValues;
  }
